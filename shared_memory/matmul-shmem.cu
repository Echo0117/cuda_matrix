...
__global__ void multiplyMatrixGPUByBlocksThreads2DNonMultipleSharedMemory(float *dA, float *dB, float *dC,int n)
{
  ...
  int i = threadIdx.x + blockIdx.x * BSXY;
  int j = threadIdx.y + blockIdx.y * BSXY;
  
    float c = 0.0f;
    for (int numTails = 0; numTails < n; numTails+=BSXY){
      shA[threadIdx.x][threadIdx.y] = dA[i * n + threadIdx.y + numTails];
      shB[threadIdx.y][threadIdx.x] = dB[j * n + threadIdx.x + numTails];
      __syncthreads();
      
      for (int k = 0; k < BSXY; k++){
        if (i < numTails && j < numTails){
        c += shA[threadIdx.x][k]*shB[threadIdx.y][k];
      }
      __syncthreads();
    }
    dC[i * n + j] = c;
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < N; j++) {
      std::cout << std::setw(5) << std::setfill(' ') << d[i * N + j];
    }
    std::cout << std::endl;
  }
    ...
    for (int i = 0; i < 5; i++) {
    for (int j = 0; j < N; j++) {
      std::cout << std::setw(5) << std::setfill(' ') << C[i * N + j];
    }
    std::cout << std::endl;
  }
